...
void DrawLimitPlot_Combined()
{
  ...
  double mass[nPoints]={270, 300, 350, 400, 450, 500, 550, 600, 650, 700, 800, 900, 1000, 1100};
...
if(i<11){    getline(file, line); obs[i]=atof(line.substr(line.find("<")+1).c_str())*1000;
 ...
 getline(file, line); obs[i]=atof(line.substr(line.find("<")+1).c_str())*100;
  ...
  g_xsec->SetTitle("95% CL upper limits on a di-Higgs resonance decaying to 4 b-quarks; M_{X} [GeV/c^{2}]; #sigma(X) #times BR(X#rightarrowH(b#bar{b}) H(b#bar{b})) [fb]");
  //g_xsec->SetTitle("");	
...
//  divideGraph(g_bbgg, 0.00266);
  ...
  if (graviton) {leg->AddEntry(g_graviton, "RS1 KK Graviton, #tilde{k} = 0.2"); leg->AddEntry(g_radion, "Radion, #Lambda = 1 TeV");}
