...
int HbbHbb_bTagging(std::string templ, std::string sample, std::string tagString, std::string csvReshape="", std::string PUWeight="", int bJetRegression=0)
{
  ...
  TH1F *h_X_mass_bTagged_right=new TH1F("h_X_mass_bTagged_right", "X mass (after b-tagging) matched; mass (GeV)", 100, 0., 1200.);
  TH1F *h_X_mass_bTagged_wrong=new TH1F("h_X_mass_bTagged_wrong", "X mass (after b-tagging) not matched; mass (GeV)", 100, 0., 1200.);
  ...
  std::string outfilename=sample+"_selected"+"_bTagged_.root"; 
  ...
  for (int i=0; i<nEvents; ++i)
  {
    ...
    if (JetCSV[H1jet1_i]>bTagCSV_looseCut) {++nTagsL;jetTag[0]=1;} 
    if (JetCSV[H1jet2_i]>bTagCSV_looseCut) {++nTagsL;jetTag[1]=1;} 
    if (JetCSV[H2jet1_i]>bTagCSV_looseCut) {++nTagsL;jetTag[2]=1;} 
    if (JetCSV[H2jet2_i]>bTagCSV_looseCut) {++nTagsL;jetTag[3]=1;} 
    ...
    if(nTags<2) std::cout<<"Event "<<i<<"  0 "<<JetCSV[H1jet1_i]<<"  1  "<<JetCSV[H1jet2_i]<< "   2   "<<JetCSV[H2jet1_i]<<"  3  "<<JetCSV[H2jet2_i]<<std::endl;      
       ...
       )
    {
      ...
      if(matched==1) h_X_mass_bTagged_right->Fill(X_mass, weightPU);
      else h_X_mass_bTagged_wrong->Fill(X_mass, weightPU);	
  ...
  h_X_mass_bTagged_right->Write();
  h_X_mass_bTagged_wrong->Write();
