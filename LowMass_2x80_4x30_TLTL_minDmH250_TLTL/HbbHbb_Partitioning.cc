...
int HbbHbb_Partitioning(std::string sample, double h_mass, int kinConstraint=0)
{
  ...
  TH1F *h_X_mass_SR=new TH1F("h_X_mass_SR", "X mass; mass (GeV)", 100, 0., 1200.);
  TH1F *h_X_mass_VR=new TH1F("h_X_mass_VR", "X mass; mass (GeV)", 100, 0., 1200.);
  ...
  for (int i=0; i<nEvents; ++i)
  {
    ...
    */
    ...
    for(int k=0; k<4; k++){
	//std::cout<<csv[k]<<std::endl;
	if(csv[k]>fCSV) fCSV=csv[k];
	else if(csv[k]>sCSV) sCSV=csv[k]; 		
	//std::cout<<csv[k]<<"   "<<fCSV<<"   "<<sCSV<<std::endl;
		}
